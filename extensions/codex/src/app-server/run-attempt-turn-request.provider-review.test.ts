...
describe("native acknowledged turn requests", () => {
  ...
  it("rejects a continuation if its native model changes during host acknowledgment", async () => {
    const host = createAcknowledgment();
    const attempt = await prepare(host.acknowledgment);
    host.acceptNativeTurn.mockImplementationOnce(async () => {
      await Promise.resolve();
      attempt.resources.state.thread.model = "changed-model";
    });
