...
`chokidar@5.0.0` has an approved native subscription ownership patch. Optional `nativeWatchNamespace` object identity scopes native event forwarding and persistent handle sharing without changing polling or nonpersistent acquisition options. Nonpersistent handles stay dedicated, and their synthetic-child fallback reaches only the selected family while the parent is active. Persistent watchers in one namespace still share handles. Retired normal, raw, error, and synthetic-child callbacks cannot reach a replacement container, including delayed Windows `EPERM` checks. Closers are registered synchronously at acquisition for all watch modes, before async directory returns or reentrant file-add callbacks can close the owner; relative and symlink unwatch retain their logical keys. Returned polling disposers remain idempotent when the enrolled owner later closes, so they cannot retire a replacement subscription. This closes a registration gap, not an OS close-event join guarantee.

The published integrity is `sha512-TQMmc3w+5AxjpL8iIiwebF73dRDF4fBIieAqGn9RGCWaEVwQ6Fb2cGe31Yns0RRIzii5goJ1Y7xbMwo1TxMplw==`; the pnpm-generated patch SHA-256 is `63365ee46df59d8d64001d09552fcbadc2b5a672dd7d64b2ad3d25ea98526583`.

| Target | Published SHA-256 | Patched SHA-256 |
| --- | --- | --- |
| `handler.js` | `dbc95cbb1f149acf9155fc605a5a6aeeecf354aeff7606192378d71778126450` | `d1d80133c592fcd65bfb1dc388f82d24686183614f09317f7e1638166be03a44` |
| `index.d.ts` | `7c06703b5cba08462692c2df83b4ec2faedac46ecd3a6a83240db8729f0efa71` | `105e04c02915b6f670233d32f00dec3c0d1641d6830a0bb54b96253c977800c5` |
| `handler.d.ts` | `97a50b01aa5289ea4e360a4d86f2dd8fed81a03ad657439b369dcace978feca2` | `33bafd9ba9f80f375177184a07386bd7d3026de13d3b3ae60cc20bedf6f00fe2` |

The root npm bundle carries Chokidar and its `readdirp` closure with their original licenses. The source-owned package lifecycle stages this physical pair under its existing docs-map lock: packing pnpm links directly can omit the dependency or retain excluded `.pnpm` paths. Postpack restores the original module slot; `node scripts/openclaw-postpack.mjs` also recovers an interrupted pack, refusing changed staged bytes or a replaced backup. Build imports remain external so core and bundled plugin imports resolve the same installed package; separately installed plugin copies are not guaranteed to share native caches. Remove the patch, exact-version exception, bundle staging, and patch-specific package checks when an upstream release passes `test/scripts/chokidar-native-namespace.test.ts` and `test/scripts/check-openclaw-package-tarball.bundled-chokidar.test.ts`, including independent native families, retired/error callbacks, startup-close registration, returned polling disposers, logical unwatch keys, and npm/pnpm pack followed by offline installation. Skills ancestor-loss recovery remains a separate consumer change.
