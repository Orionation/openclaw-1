...
export function createPluginServiceNodeInvoker(options: {
  ...
  const current = capturePluginLifecycleAuthority(registry, record);
  const epoch = capturePluginRegistryLifecycleEpoch(registry);
  const registrySignal = capturePluginRegistryLifecycleSignal(registry, epoch);
  if (!current || !registrySignal) {
    return undefined;
  }
  const signals = [lifetime.signal, registrySignal];
