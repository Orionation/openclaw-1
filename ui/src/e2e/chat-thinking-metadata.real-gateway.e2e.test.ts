...
const harnessOnlyLevels = [{ id: "ultra", label: "ultra" }];
async function expectUnselectedUltraOption(page: Page) {
  const options = await page
    .locator(".chat-controls__model-settings:visible [data-chat-thinking-option]")
    .evaluateAll((buttons) =>
      buttons.map((button) => ({
        value: button.getAttribute("data-chat-thinking-option"),
        pressed: button.getAttribute("aria-pressed"),
      })),
    );
  expect(options).toEqual([{ value: "ultra", pressed: "false" }]);
}
...
suite.define(() => {
  it("retains saved Off through a real model change to harness-only Ultra", async () => {
        ...
        async ({ page }) => {
          ...
          await expect
            ...
            .toMatchObject({
              ...
              model: "no-effort",
              thinkingLevel: "off",
              thinkingLevels: harnessOnlyLevels,
            });
          ...
          await expectUnselectedUltraOption(page);
  ...
  it("reports harness-only Ultra without advertising a default or changing the session", async () => {
    const commands: unknown[] = [];
    const call = async (method: string, params: Record<string, unknown>) => {
      const result = await instance.cli([
        "gateway",
    ...
    try {
      ...
      expect(model?.thinkingLevels).toEqual(harnessOnlyLevels);
      ...
      expect(row?.thinkingLevels).toEqual(harnessOnlyLevels);
      expect(row?.thinkingDefault).toBeUndefined();
      expect(before.defaults.thinkingLevels).toEqual(harnessOnlyLevels);
